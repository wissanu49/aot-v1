...
class StatusNotifier {
	...
	protected $status_new;
		...
		$this->save_logs = psm_get_conf('log_status');
                /////////// LINE SETTING  //////////////////
                $this->send_line = psm_get_conf('line_status');
	}
	...
	public function notify($server_id, $status_old, $status_new) {
		...
		$this->server = $this->db->selectRow(PSM_DB_PREFIX . 'servers', array(
			...
			'server_id', 'ip', 'port', 'label', 'type', 'pattern', 'status', 'error', 'active', 'email', 'sms', 'pushover', 'line'
		));
		...
		if($this->send_pushover && $this->server['pushover'] == 'yes') {
			...
			$this->notifyByPushover($users);
                ...
                // check if line is enabled for this server
		if($this->send_line && $this->server['line'] == 'yes') {
			// yay lets wake those nerds up!
                         //die($this->server['line']);
                        $this->notifyByLine();
	...
	protected function notifyByTxtMsg($users) {
        ...
        protected function notifyByLine() {
		//$line = new \psm\Line();
                
                $this->_token = psm_get_conf('line_token');               
                ...
                curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query(array('method' => 'notify','acctoken' => $this->_token, 'massege' => $message)));
